...
/// Per-session directory for process output files.
fn session_dir(app: &AppHandle, session_id: &str) -> Result<PathBuf> {
    let dir = app
        .path()
        .app_data_dir()
...
pub async fn ensure(
    ...
    tauri::async_runtime::spawn(tail_session(Tail {
        app: app.clone(),
        store: store.clone(),
        session_id: session.id.clone(),
        proc_id,
...
impl Liveness {
    fn is_dead(&mut self) -> bool {
        match self {
            ...
            Liveness::Pid(pid) => !crate::platform::process_alive(*pid),
            Liveness::Dead => true,
        }
    }
}
    ...
    fn handle_line(&mut self, line: &str, offset_after: u64) {
        ...
        for event in parsed.events {
            if event.is_transient() {
                if let AgentEvent::TextDelta { text } = &event {
                    emit_delta(&self.app, &self.session_id, text);
                }
                continue;
            }
        ...
        if let Some(cost) = parsed.cost_usd {
            ...
            if self
                ...
                .unwrap_or(false)
            {
                let _ = self.store.record_turn(&self.session_id, cost);
                let _ = self
                    .store
...
pub fn recover(app: AppHandle, store: Store) {
    tauri::async_runtime::spawn(async move {
        ...
        for row in procs {
            ...
            let alive = crate::platform::process_name(row.pid)

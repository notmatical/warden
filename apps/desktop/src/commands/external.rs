...
//! what's actually installed. Pure shell glue over `tauri_plugin_opener` + the
//! `which` PATH probe; nothing here belongs in `warden-core`.
...
fn app_installed(def: &AppDef) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Some(home) = warden_core::util::home_dir() {

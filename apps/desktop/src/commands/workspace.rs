//! Tauri command wrappers for workspace structure: projects, groups, per-session
//! root selection, and per-repo worktree config. The logic lives in
//! `warden_core::{store, git, workspace::config}`.

use std::path::Path;

use tauri::State;

use warden_core::workspace::config::{self, WorktreeConfig};
use warden_core::workspace::{Group, Project};
use warden_core::{git, AppError, CommandResult, Session};
...
pub async fn open_project(state: State<'_, AppState>, path: String) -> CommandResult<Project> {
    ...
    state
        .store
        .upsert_project(&name, &path, git::is_repo(p))
        .map_err(Into::into)
}
...
/// Drop blank entries and trim whitespace from a command list.

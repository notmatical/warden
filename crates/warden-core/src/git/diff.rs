...
/// Sum added/removed line counts from `git diff --numstat` output. Binary files
/// report `-` for both counts and are skipped. Shared by `cli::uncommitted_lines`
/// and `lines_vs_base`.
pub(super) fn parse_numstat(out: &str) -> (u32, u32) {
    let mut added = 0u32;
...
/// Added-line count for untracked files in `worktree` (`ls-files --others`):
/// each new file's every line counts as an add. Unreadable (binary) files
/// contribute nothing. Shared so untracked work shows in the change counter.
pub(super) fn untracked_added_lines(worktree: &Path) -> u32 {
    let mut added = 0u32;
...
/// entry per file with its patch. Untracked files are included with a
/// synthesized all-add patch.
...
pub fn lines_vs_base(worktree: &Path, base: &str) -> (u32, u32) {
    let (mut added, removed) = run(worktree, &["diff", base, "--numstat"])
        .map(|out| parse_numstat(&out))
        .unwrap_or((0, 0));
    added += untracked_added_lines(worktree);
    (added, removed)
}

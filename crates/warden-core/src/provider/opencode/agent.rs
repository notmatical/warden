...
use crate::event::{emit_delta, emit_event};
use crate::session::Session;
use crate::store::Store;
use crate::{AgentEvent, EffortLevel, PermissionMode, TokenUsage, ToolDenial};
...
static TURNS: LazyLock<TurnRegistry<TurnHandle>> = LazyLock::new(TurnRegistry::new);
...
static ASKS: LazyLock<TurnRegistry<Vec<PendingAsk>>> = LazyLock::new(TurnRegistry::new);
...
pub fn has_pending_question(session_id: &str) -> bool {
    ASKS.get_cloned(session_id).is_some_and(|list| {
        list.iter()
...
fn drop_ask_by_request(request_id: &str) {
    for list in ASKS.lock().values_mut() {
        list.retain(|a| {
            let id = match a {
...
pub async fn answer_question(store: &Store, session_id: &str, text: &str) -> Result<()> {
    let turn = TURNS
...
pub async fn run_turn(
    ...
    let outcome = loop {
        ...
        let sse = tokio::spawn(tail_events(SseContext {
            store: store.clone(),
...
/// Token usage from a `step-finish` part's `tokens` object. OpenCode nests cache
/// reads/writes under `tokens.cache`, so the flat [`TokenUsage::from_keys`]
/// helper covers input/output and the cache pair is read from the nested object.
        ...
        "message.part.delta" => {
            ...
            match parts.get_mut(part_id) {
                Some(PartTrack::Text { emitted }) => {
                    *emitted += delta.len();
                    emit_delta(&ctx.session_id, delta);
                }
...
fn handle_part_snapshot(ctx: &SseContext, part: &Value, parts: &mut HashMap<String, PartTrack>) {
    ...
    match part.get("type").and_then(Value::as_str).unwrap_or_default() {
            ...
            match parts.get_mut(part_id) {
                Some(PartTrack::Text { emitted }) => {
                    if text.len() > *emitted && text.is_char_boundary(*emitted) {
                        emit_delta(&ctx.session_id, &text[*emitted..]);
                    }
        ...
        "tool" => {
            ...
            persist(&ctx.store, &ctx.session_id, tool_use);
            if let Some(result) = tool_result {
                persist(&ctx.store, &ctx.session_id, result);
            }
        }
        _ => {}
    }

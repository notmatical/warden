...
//! correlation, notification routing) lives in [`crate::provider::jsonrpc`];
...
use crate::agent::settle::{persist_event as persist, set_status};
use crate::error::{AppError, Result};
use crate::event::emit_delta;
use crate::event::text::clip;
use crate::event::TokenUsageKeys;
use crate::provider::jsonrpc::Client;
use crate::session::{Session, SessionStatus};
use crate::store::Store;
use crate::{AgentEvent, EffortLevel, PermissionMode, TokenUsage};
...
/// In-flight turns: session id → (threadId, turnId). The turnId is empty until
/// `turn/started`; used to send `turn/interrupt` on cancel.
static TURNS: LazyLock<TurnRegistry<(String, String)>> = LazyLock::new(TurnRegistry::new);
...
pub fn interrupt(session_id: &str) -> bool {
    let Some((thread_id, turn_id)) = TURNS.get_cloned(session_id) else {
        return false;
    };
    if turn_id.is_empty() {
        return false;
...
pub(super) fn codex_effort(effort: EffortLevel) -> &'static str {
    match effort {
...
fn codex_usage(turn: Option<&Value>) -> Option<TokenUsage> {
    ...
    TokenUsage::from_keys(
        ...
        &TokenUsageKeys {
            ...
            cache_read: &["cachedInputTokens", "cached_input_tokens"],
            cache_creation: &[],
        },
    )
}
...
pub async fn run_turn(
    ...
    while let Some(note) = rx.recv().await {
        if handle_notification(store, &session.id, &note.method, &note.params) {
            finished = true;
            break;
        }
    }
...
fn handle_notification(store: &Store, session_id: &str, method: &str, params: &Value) -> bool {
    match method {
        "turn/started" => {
        ...
        "error" => {
            let message = params
                ...
                .to_string();
            persist(store, session_id, AgentEvent::Error { message });
            ...
            if retrying {
                ...
                set_status(store, session_id, SessionStatus::Error);
                true
            }
        }
        _ => false,

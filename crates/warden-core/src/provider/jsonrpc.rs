...
/// A server notification routed to a subscriber. Named `RpcNotification` to
/// avoid colliding with the user-facing `event::Notification` payload.
...
pub struct Client {
    ...
    routes: Mutex<HashMap<String, mpsc::UnboundedSender<RpcNotification>>>,
    route: RouteFn,
...
impl Client {
    ...
    pub fn spawn(name: &'static str, mut cmd: Command, route: RouteFn) -> Result<Arc<Self>> {
        ...
        tokio::spawn(reader_loop(client.clone(), stdout));
        tokio::spawn(drain_stderr(client.clone(), stderr));
        Ok(client)
    }
    ...
    pub fn subscribe(&self, key: &str) -> mpsc::UnboundedReceiver<RpcNotification> {
    ...
    fn routes_map(
        &self,
    ) -> std::sync::MutexGuard<'_, HashMap<String, mpsc::UnboundedSender<RpcNotification>>> {

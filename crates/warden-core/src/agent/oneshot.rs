...
use crate::provider::claude::agent::{resolve_claude, run_oneshot};
use crate::Backend;

/// Dispatch a one-shot to a backend's cheap workflow model. The single per-turn
/// dispatch point — each backend's concrete invocation lives in its own `run_*`
/// (Claude/Codex here, OpenCode in its HTTP adapter), keyed by [`Backend`].
...
pub(crate) async fn run_claude(working_dir: &Path, prompt: &str) -> Option<String> {
...
pub(crate) async fn run_codex(working_dir: &Path, prompt: &str) -> Option<String> {

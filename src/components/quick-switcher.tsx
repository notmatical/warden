...
function QuickSwitcherSession({ sessionId }: { sessionId: string }) {
	...
	return (
		<button
			...
			title={session.title}
			className={cn(
				"flex h-6 w-full items-center gap-2 rounded-lg px-2 text-xs transition-colors",
				active
...
function QuickSwitcherGroup({
	...
	const sessionIds =
		useAppStore((s) => s.sessionsByGroup[groupId]) ?? EMPTY_IDS;
	...
	return (
		<div className="flex flex-col gap-0.5">
			<button
				...
				title={name}
				className={cn(
					"flex h-7 w-full items-center gap-1.5 rounded-lg px-2 text-sm transition-colors",
					active
						? "text-foreground"

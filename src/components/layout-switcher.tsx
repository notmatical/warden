	...
	const setMode = (next: LayoutMode) => {
		...
		if (next !== "single") {
			...
			const queue = [active, ...(tabsByGroup[activeGroupId] ?? [])].filter(
				(id): id is string =>
					!!id && !placed.has(id) && !seen.has(id) && !!seen.add(id),
			);
			resized = {
				...resized,
				...
				<div className="flex items-center gap-1">
					{MODES.map(({ mode: m, label, Icon }) => (
						<button
							...
							className={cn(
								"flex size-8 items-center justify-center rounded-lg text-muted-foreground outline-hidden transition-colors hover:bg-muted hover:text-foreground",

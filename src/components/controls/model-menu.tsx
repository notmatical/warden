          ...
          <div className="min-w-0 flex-1 p-1">
            {paneLocked ? (
              <p className="px-2 py-3 text-xs text-muted-foreground">
                This session runs on {paneEntry.backend === "codex"
                  ? "Codex"
                  : "Claude"}
                . Its provider is fixed once it has started.
              </p>
            ) : (
              paneModels.map((model) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-sm outline-hidden transition-colors hover:bg-accent focus:bg-accent focus:text-accent-foreground"
                  >
                    <Check
                      className={cn(
                        "size-4 shrink-0",
